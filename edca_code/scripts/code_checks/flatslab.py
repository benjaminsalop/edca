...
from edca_code.scripts.code_checks.continuousslab import (check_slab_row_preserve_math, _load_yaml)
...
# _choose_rebar_for_As_required (identical to your continuous version)
...
# run_code_check_for_typology: same wrapper as before
def run_code_check_for_typology(systems_catalog: pd.DataFrame,
    ...
    # same selection code as your continuous file
...
def slab_dimensions(slab_depth_m: float,
    ...
    """
    Flat-slab geometry: default behaviour same as continuous slab, but
    default moment-width coefficients in flexural_design are tailored to flat slab behavior.
    If you want column-strip/field-strip effective spans, supply slab_length_m as the clear span
    between columns in the selected direction.
    """
    nominal_cover = max(d_rebar_m, cover_m) + deviation_allowance_m
    # for flat slabs we typically use smaller a1/a2 (no deep beams)
...
def flexural_design(ultimate_load_kN_m2: float,
    ...
    """
    Flat slab typical coefficients:
      - interior negative moment coeff (approx) 0.08
      - positive mid span coeff (approx) 0.04
    These are rough, replace with your column-strip/field-strip factors where required.
    """
    if span_factors is None:
        span_factors = {"end": 0.08, "mid": 0.04}
