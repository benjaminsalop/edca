...
def _coerce_material_id(r: Dict[str, Any], materials_path: Optional[str]) -> None:
    ...
    for k in (
        ...
        if not _is_missing(r.get(k)):
            # preserve explicit component material ids
            r["material_concrete_id"] = r.get("material_concrete_id") or r.get("concrete_material_id") or r.get("concrete_id")
            r["material_rebar_id"] = r.get("material_rebar_id") or r.get("rebar_material_id") or r.get("rebar_id")
            r["material_screed_id"] = r.get("material_screed_id") or r.get("screed_material_id") or r.get("screed_id")

            # legacy fallback only
            if ("material_id" not in r or _is_missing(r.get("material_id"))) and not _is_missing(r.get("material_concrete_id")):
...
def _build_inputs_snapshot(r: Dict[str, Any]) -> Dict[str, Any]:
    ...
    snap["material_concrete_id"] = r.get("material_concrete_id")
    snap["material_rebar_id"] = r.get("material_rebar_id")
    snap["material_screed_id"] = r.get("material_screed_id")
    ...
    *,
    ...
    for _, row in candidates_df.iterrows():
        ...
        try:
            ...
            "materials": {
                "concrete": _material_debug(material_csv_path, r.get("material_concrete_id") or r.get("material_id")),
                "rebar": _material_debug(material_csv_path, r.get("material_rebar_id")),
                "screed": _material_debug(material_csv_path, r.get("material_screed_id")),
            },
            }
