...
def slab_dimensions(slab_depth_m: float,
                    ...
                    flange_thickness_m: Optional[float] = None,
                    rib_depth_m: Optional[float] = None) -> Dict[str, float]:
    """
    Ribbed-slab geometry:
      - flange_thickness_m: thickness of the top flange (the slab topping)
      - rib_depth_m: depth of ribs (distance from bottom of flange to soffit of rib)
    Effective depth for the bottom reinforcement is taken from bottom of flange to centroid of bottom bar,
    so we account for flange + rib height.
    If flange_thickness_m not provided, it falls back to slab_depth_m (treat as solid).
    """
    nominal_cover = max(d_rebar_m, cover_m) + deviation_allowance_m
    if flange_thickness_m is None:
        flange_thickness_m = slab_depth_m  # treat as solid slab if flange not given
        rib_depth_m = 0.0 if rib_depth_m is None else rib_depth_m
    else:
        rib_depth_m = 0.0 if rib_depth_m is None else rib_depth_m

    # a1/a2 same heuristic
    a1 = min(flange_thickness_m / 2.0, wall_thickness_m / 2.0)
    a2 = a1

    # effective_span uses clear span + edge allowances
    effective_span = a1 + a2 + slab_length_m

    # effective depth for tensile (bottom) reinforcement:
    # - assume bottom reinforcement sits in ribs under flange, so distance from top of flange
    #   to centroid of bottom bar = flange_thickness + rib_depth - nominal_cover - d_rebar/2
    effective_depth = flange_thickness_m + rib_depth_m - nominal_cover - d_rebar_m / 2.0

    # if that gives <=0 fall back to a conservative solid-slab formula
    if effective_depth <= 0:
        effective_depth = slab_depth_m + screed_depth_m - nominal_cover - d_rebar_m / 2.0

    if effective_depth <= 0:
        raise DesignError("Effective depth non-positive: check rib/flange depths / covers / rebar diameter.")
...
def flexural_design(ultimate_load_kN_m2: float,
    ...
    """
    Ribbed slab: because ribs are usually analysed as one-way elements (rib spacing defines width),
    we default to one-way coefficients (simply-supported-like mid-coef = 0.125 for single span; for continuous ribbed strips adjust manually).
    If using a ribbed slab as two-way, override span_factors when calling.
    """
    if span_factors is None:
        # default to one-way behaviour for ribs
