...
def load_systems_catalog(
    ...
    def legacy_floor_catalog(parent: Path) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame], Optional[pd.DataFrame]]:
        floor_variants = None
        floor_families = None

        for ext in (".parquet", ".pq", ".csv"):
            floor_variants = load_try(parent / f"floor_variants{ext}")
            if floor_variants is not None:
                break

        for ext in (".parquet", ".pq", ".csv"):
            floor_families = load_try(parent / f"floor_families{ext}")
            if floor_families is not None:
                break

        if floor_variants is None:
            for ext in (".parquet", ".pq", ".csv"):
                floor_variants = load_try(parent / f"floor_systems{ext}")
                if floor_variants is not None:
                    break

        if floor_variants is None:
            return None, floor_families, None

        variants = floor_variants.copy()
        variants = variants.rename(
            columns={
                "floor_variant_id": "system_variant",
                "floor_family_id": "system_family",
                "floor_system_id": "system_family",
            }
        )

        families = floor_families.copy() if floor_families is not None else None
        if families is not None:
            families = families.rename(columns={"floor_family_id": "system_family"})

        systems = variants
        if families is not None and "system_family" in systems.columns and "system_family" in families.columns:
            family_cols = [
                c
                for c in families.columns
                if c == "system_family" or c not in systems.columns
            ]
            systems = systems.merge(families[family_cols], on="system_family", how="left")

        if "system_variant" not in systems.columns and "system_family" in systems.columns:
            systems = systems.copy()
            systems["system_variant"] = systems["system_family"].astype(str)

        return systems, families, variants
    ...
    if systems_df is None:
        systems_df, fallback_families, fallback_variants = legacy_floor_catalog(parent)
        if families_df is None:
            families_df = fallback_families
        if variants_df is None:
            variants_df = fallback_variants
...
def filter_systems(
    ...
    *,
    ...
    if min_span_required is None:
        ...
        # ── panel-span bypass: secondary-beam-supported slabs (two-way, beam-and-block,
        # composite deck) span only to the secondary beam, not the full bay.  Their
        # catalog max_span reflects the panel span, so skip the bay-span check.
        if "beam_requirements" in df.columns:
            is_sec = df["beam_requirements"].fillna("").str.strip().str.lower() == "secondary"
            span_mask = span_mask | is_sec
    ...
    # ── panel-span bypass for secondary-beam-supported floors ──────────────
    # Two-way slabs, beam-and-block, etc. with beam_requirements="secondary"
    # span only the secondary beam spacing (~1.5m), not the full bay.  The
    # catalog sdl/ll entry reflects a much longer span than the actual panel
    # span, so any depth is structurally adequate.  Pass all such systems
    # regardless of catalog total_capacity.
    if "beam_requirements" in df.columns:
        is_sec = df["beam_requirements"].fillna("").str.strip().str.lower() == "secondary"
        load_mask = load_mask | is_sec
