...
def build_load_context(
    ...
    *,
    ...
    if load_values_yaml is None:
        ...
        if not Path(load_values_yaml).exists():
            load_values_yaml = Path("inputs/source/presets/loads/load_values.yaml")
    if load_combinations_yaml is None:
        load_combinations_yaml = occ_path.with_name("load_combinations.yaml")
        if not Path(load_combinations_yaml).exists():
            load_combinations_yaml = Path("inputs/source/presets/loads/load_combinations.yaml")
    ...
    for f in range(int(num_floors)):
        ...
        # Convert imperial occupancy loads from psf → kPa.  The occupancies CSV
        # stores ASCE loads in psf (pounds per square foot); all downstream
        # structural checks expect kPa.  1 psf = 0.047880 kPa.
        _occ_unit = str(r.get("unit", "") or "").strip().lower()
        if _occ_unit == "imperial":
            _PSF_TO_KPA = 0.047880
            raw_sdl *= _PSF_TO_KPA
            raw_ll  *= _PSF_TO_KPA
            logger.info(
                "[loads] Converted imperial loads (psf→kPa) for floor %s (%s): "
                "SDL=%.3f kPa, LL=%.3f kPa",
                f, occ, raw_sdl, raw_ll,
            )
